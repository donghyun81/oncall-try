...
- 비상 근무 월과 시작 요일 입력 기능 .
  - 비상 근무를 배정할 월과 시작 요일의 입력 값이 올바르지 않은 경우, '비상 근무를 배정할 월과 시작 요일'부터 다시 입력받는다.
  - 월은 1~12월이 존재하고 2월은 28일까지 존재한다.
  - 근무 요일은 "월화수목금토일"을 입력 받는다.
- 평일 비상 근무 순번데로 사원 닉네임 입력
  - 닉네임은 ","로 구분해서 입력 받는다.
...
- 평일 근무 사원 객체를 만든다.
- 휴일 근무 사원 객체를 만든다.
- 날짜 객체를 만들고 요일과 사원을 속성으로 받는다.
- 비상 근무 배정 월 객체를 만들고 날짜를 속성으로 받는다
- 월 객체의 날짜 속성을 만든다.
- 월 객체에 있는 날짜 객체에 사원 객체들을 배치한다.
- 월 객체에 있는 날짜를 통해서 비상 근무표를 출력한다.